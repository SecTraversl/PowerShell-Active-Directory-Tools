...
.EXAMPLE
  ...
  DistinguishedName : OU=Help Desk,OU=Users,OU=Controlled Objects,DC=subd,DC=MyDomain,DC=com
  Name              : Help Desk
...
function Get-ADObjectInfo {
  ...
  [Alias('ADObjectInfo')]
  param (
    [Parameter(Mandatory, HelpMessage = 'Reference a partial or full object name to retrieve from Active Directory.')]
    [string[]]
    $Name, 
    [Parameter()]
    [string[]]
