...
.EXAMPLE
  ...
  Here we run the function by calling its built-in alias of "ADUserLastLogonSimple".  In return, we get various Logon information for the user according to the Domain Controller found in the $env:LOGONSERVER variable.  This is not necessarily the most authoritative timestamp of domain Logon information for that user -- in order to get that you must query each Domain Controller and find the most recent timestamp.

.INPUTS
.OUTPUTS
.NOTES
  Name:  Get-ADUserLastLogonSimple.ps1
  Author:  Travis Logue
  ...
  .
#>
function Get-ADUserLastLogonSimple {
  [CmdletBinding()]
  [Alias('ADUserLastLogonSimple')]
  param (
